...
const useSize: (dom: HTMLElement | null, ms?: number) => ISize = (dom: HTMLElement | null, ms: number = 500): ISize => {
  const [size, setSize] = useState({ width: 0, height: 0 });
  const handler: React.MutableRefObject<number> = useRef(0);
  const observer: React.MutableRefObject<ResizeObserver> = useRef(new ResizeObserver((entries: ResizeObserverEntry[]): void => {
